 ...
 * an operator is actually looking at. It is also the reason this module still has
 * a derivation where the other panels' modules pass the message straight through:
...
export function useLineOutageSocket() {
  const sock = useServerSocket<Wire['LineOutageLog']>(LINE_OUTAGE_WS_PATH)

  const state = $derived<LineOutageState>(
    sock.message === null
      ? null
      : {
          ...sock.message,
          events: sock.message.events.map((event) => ({
            ...event,
