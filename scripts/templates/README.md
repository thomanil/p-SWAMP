...
so keep the text after a token on any line short, and prefer a token at the end
of its line. The prose here is written to the script's 32-character name cap; the
one line that does not fit at the very top of that range is `state_message`'s
signature, which carries `__NAME__` twice.
