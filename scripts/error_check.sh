...
# Read with a `while read -d ''` loop rather than `mapfile -d ''`: mapfile is
# bash 4+ (and its -d flag 4.4+), while macOS still ships bash 3.2 as /bin/bash.
# There it failed with "mapfile: command not found", leaving PY_FILES unset — and
# because the summary only counts entries in FAILURES, the three Python checks
# below were skipped while the script still printed "All checks passed" and
# exited 0. Keep this loop POSIX-ish; don't reintroduce a bash-4 builtin.
PY_FILES=()
