...
Context captured along the way: how the grid monitor got built, what it cost,
what is left, and where the Python should live once it is done.

`AGENTS.md` describes the system as it *is*; this file describes how it got there,
why, where the traps are, and what happens next. Read this if you are re-doing,
extending, or finishing the port, or deciding where the code lives; read
`AGENTS.md` if you are changing the result.
...
discipline of the work was that it modified that checkout *not at all* — p-SWAMP
was a library, reached through a uv path dependency.

**Those two are now one repo** (this one): the harness under `app/`, the desktop
package at the root, the port reapplied on top. The port itself is unchanged; only
the seam between the two halves moved:
...
The `/workspace` depth trick survives, for the same reason as before (§4.1): uv
won't normalise a relative path above its base directory, so the server can't be
flattened to `/app` while `../../` has to mean the same thing on a laptop and in
the image.

**The "changes nothing upstream" rule is now a design rule, not a fact of the
filesystem** — there is no separate upstream to protect. The discipline still
holds in the code (`pswamp_web/` imports `pswamp.*`; nothing under root `src/`
imports from `app/`) and is worth keeping, because it is what keeps *either* of
§7's moves cheap. But only review enforces it now.
...
still in `src/pswamp/`, listed in §11. Don't "simplify" either away without fixing
the gap first. The first two should arguably live in the core eventually, and are
written so they can move.

**`synchrophasor` is deliberately excluded from the image.** A declared base
dependency, but only the live-PMU and playback paths import it, and it's the one
dependency fetched from git — excluding it keeps git, network and a hashless VCS
pin out of the build. The Dockerfile's `import server` smoke test makes that a
checked decision; keep it. §10.2 puts it back with the live PMU path.
...
from 176 channels and 1.0 MB. The extra 524 are `i_Magnitude`/`i_Angle`, for one
reason — `LineOutageDetectionApp` reads `i_Magnitude` and nothing else does.
Without them it runs and detects nothing, forever — worse than not running it.

The simulation is otherwise identical, and the generator's `verify()` proves it:
same median frequency, voltage, islanded stations. Nothing downstream changed —
the time-window stream is bounded by the client's channel selection, not the
recording's width, so the measured bandwidth below is unchanged.

**Regenerating needs less than the `[full]` extra:** a Python 3.11 venv with the
root package + `tops-rt` + `synchrophasor`, no Qt. One wart: the tool also needs
`fastapi` on the path, purely because it imports `pswamp_web.recorded_io` and that
package's `__init__` pulls in the whole web stack. Worth fixing in the tool.
...
The path dependency resolved on the host but failed in the image with *"cannot
normalize a relative path beyond the base directory"* — uv does **not** clamp at
`/` the way a shell does, so the plan's assumed `/app/../../` → `/` was wrong.
...
and `../../` is correct in both places, with no image-only manifest. This is why
the image doesn't flatten the server to `/app`, and the single largest piece of
machinery §9 would delete.
...
not, so a channel close to two references appeared in both groups. Consumers
recovering the main system by subtraction got far fewer stations than were
connected — "main = 3 stations" while the grid was intact.
...
assessments, groups summing to exactly 44 stations. The real fix is two lines in
`detect_islands`; see §11.

Only visible because the port *printed real island membership early* — print
domain data as soon as the pipeline runs.
...
Cost the most time, and produced a **falsely reported regression**. Headless
Chrome's virtual time fast-forwards timers but **not real network I/O**, so the
last WebSocket initiated is cut off mid-handshake — non-deterministically, by
machine load. Raising the budget doesn't help, which is what makes it look like a
logic bug rather than a measurement artefact.

**Measure real network behaviour only with real wall-clock time:** launch the
browser detached, `sleep`, inspect. See §5.
...
timeline (one `RecordingPlayer`, one measurement window, one detector) — and it
was a *less* faithful port than a dashboard, since the Qt UI (`gui/main_window.py`)
is one `QMainWindow` with a central grid view and docks. The client/server symmetry
is real on the *server*, false on the client. Cost: a whole refactor, plus two bugs
that only appeared once the panels were adjacent (island colours disagreeing at
index 0; `NavLink` matching everything).
...
**True either way, and worth protecting:** the `pswamp_web/` package is
self-contained — no module imports from the rest of the web backend (only stdlib,
fastapi, pydantic, numpy and `pswamp.*`), and every intra-package import is
relative. That is what makes both moves cheap, and the one invariant a change must
not break.
...
separate. It gets `app/` to mean "all the code" without committing either way, at
the cost of one extra `git mv` whichever way §10 goes.
...
panels. It proves the architecture but does not yet replace `gui/main_window.py`.
...
**This was designed for.** `hub.py` passes `io=self.player.subscribe(...)` into
each application — a constructor argument. Swapping in `kafka_io` is a wiring
change at that one line, exactly the bet §2 recorded. Don't let it become a
rewrite.

**The consequences around it are the hard part**, none of which the replay path
exercises:
