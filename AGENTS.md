...
monitoring applications. Beside it sit the other pages, each its own subapp: the
deliberately trivial reference example, which keeps the "adding a page" path
honest, and the worked examples and load tests of the shared core:
...
- **`/frequency-peek` is the smallest module-and-page over the core.** One
  pipeline per client over the streamer's two providers
  (`FREQUENCY_PEEK_DATA_CLIENTS` names others), whose player goes **live** as
  soon as it starts (an autoplaying replay if no live source is configured),
  and one `FrequencyModule` (`frequency_module.py`) reducing each `PmuFrame` to
  the per-station frequencies. **Live only, so no commands**: no transport
  controls and no POSTs; state goes down the socket and nothing comes up.
...
dependencies = ["fastapi…", "uvicorn[standard]…", "p-swamp",
                "pswamp-core[kafka,remote-data]", "nfoursid…"]
...
- **`app/server-python/`** — the authoritative state server. The code lives in
  ...
  **`src/pswamp_web/`** — see "The p-SWAMP web layer" below; it is a package of
  ...
  service.
  Beside it sit the other pages' packages (one per page listed under "What
  this is", plus `errors/`). **`src/reference_subapp/`** is the
  ...
  *simulated* PMU records extracted by hand from the Nordic 44 simulation that now
  ...
  package imports its domain-free helpers from (its `__all__` is the list) —
...
- **This currently does not scale past one replica.** Because state is in-process,
  the k8s manifest is `replicas: 1`. A second pod would keep its own
...
- **The mount prefix is discovered at runtime, never baked in.** Remotely the app
  ...
  WebSocket urls. This is what lets **one** image run at the origin root and
  under a prefix, which matters because there is exactly one Dockerfile and
  every deployment builds from it. Two
...
- **`/docs` is a localhost-only convenience, and the nav hides it elsewhere.**
  FastAPI's `/docs`, `/redoc` and `/openapi.json` embed root-absolute urls
  (Swagger fetches `/openapi.json`; the doc advertises its own server url), so
...
`ClientId`, `CommandAck`, `CLIENT_ID_PATTERN`/`read_client_id`, `send_state`,
`get_logger` and the `pump.py` push loop. Reading the rule as "duplicate it, then keep the copies equal" is
...
operation (`play`, `playRange`) and swallows a failure with a log — a command
...
- **The socket half is in there too**, as `components.schemas` plus an
  ...
  second spec format, a second generator and a second drift check for one-way
  channels (one per socket endpoint) that each carry a single message shape. An entry reads
...
- **`./scripts/run-python-server-tests.sh`** — the server tests under
  ...
  **stubbed Hub**, so nothing binds a port. The first suite,
  ...
  automatically (`test_*.py`; `testpaths = ["tests", "../../core/tests"]`,
  `pythonpath = ["src", "../../core/examples"]` in the server manifest), and arguments pass straight
...
./scripts/run-playwright-tests.sh                          # the Playwright browser specs under e2e/ (brings the compose stack up and down)
```
...
- **It tests the wire, not the UI.** It would not notice a button that stopped
  calling its command or a panel that renders nothing. **Playwright covers
  that**, as a start: `e2e/` is its own npm project (`playwright.config.ts`
  brings the stack up with `docker compose up --build` and tears it down after;
  one spec so far, `reference-subapp.spec.ts`), run locally by
  `./scripts/run-playwright-tests.sh` and in CI by the `playwright-e2e` job. It
  is a strict addition: this layer stays valid underneath it as the fast,
  dependency-free one.
...
- **Lint is explicitly `--select F`** (pyflakes only — real bugs, not style) in
  ...
  families (I, B, S…) are not inherited either — add one deliberately if you want
  ...
  linter.
- **CI checks; it never publishes or deploys.** The workflows (below) check pull
  requests and check that the image still builds on `main`; no image is pushed
  anywhere and nothing rolls anything out to a cluster. The **pre-push hook**
  is still the first gate and the fast one — run `error_check.sh` before finishing
  any change rather than discovering it in CI.

## CI

**Two workflows under `.github/workflows/`, one per concern. Neither publishes
anything.**

- **`quality-checks.yml`** runs on every pull request (and from the Actions tab),
  a new push cancelling the previous run. Four jobs: `static-errorcheck`
  (`error_check.sh`), `unit-tests`, `e2e-smoke-test` (builds the image, runs it,
  runs `e2e-smoke-test.sh` against it) and `playwright-e2e` (the Playwright specs
  under `e2e/`, bringing the stack up with compose). The first three are
  independent; `playwright-e2e` `needs` `unit-tests` and `e2e-smoke-test`, and
  its header calls it an initial implementation meant eventually to replace the
  smoke test. `unit-tests` runs the Python suites through their runner scripts
  (`run-python-server-tests.sh`, then `check-remote-data-service.sh` against the
  stub; the desktop `run-core-python-tests.sh` step is commented out with a TODO
  until its missing-module failure is resolved), so `error_check.sh` stays
  strictly static.
- **`build-container.yml`** runs on every push to `main` (which includes every
  merged PR) and from the Actions tab: it builds the image with the `GIT_SHA`
  build arg threaded in, as a deployment would, starts it and waits for
  `/healthz`. **It pushes nothing**: `push: false`, `load: true`, and the image
  is gone with the runner. This repo cannot host binaries, so the Dockerfile is
  an example — a downstream deployment builds and pushes its own image, with its
  own tag and registry (the workflow's header comment has the commands). It runs
  no gates of its own — `main` is protected, so every commit on it already
  passed the checks on its pull request.
...
status checks to pass", selecting the `quality-checks.yml` job names:
**`static-errorcheck`**, **`unit-tests`**, **`e2e-smoke-test`** and, once it
should gate too, **`playwright-e2e`**. Those rules match on the *job* name, not
the workflow's, so renaming a job silently un-requires it there — rename the job
and the protection rule together. (The jobs used to live in a single
`ci-pipeline.yml`, and before that the check job was called `check`; if
protection was configured against either, it needs re-selecting.)

No registry is used. Publishing to GHCR from `main` (the old
`build-and-publish.yml`), and before that a GitLab pipeline publishing to a
GitLab registry against the internal mirror's runners, were both built and then
dropped. The GitLab one is recoverable rather than guessed at, if it is ever
revived:
...
- **No `paths` filter on the `pull_request` trigger, on purpose.** The
...
- **CI builds `linux/amd64` only**, stated explicitly via `platforms:` in both
  workflows: that is what a downstream cluster would run, and dev boxes (often
  arm64) build their own image via compose/minikube, so an emulated arm64 leg
  would cost every push for a check nothing needs. The *Dockerfile* is
